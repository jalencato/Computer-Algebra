% HearnFull.red
% by JalenCato
% in "D:\DeskTop\Computer Algebra\Exp\HearnFull.red";
...
begin scalar result, flag;
   ...
   while not(xlist = {}) do <<
       ...
       >> else if not flag then <<
           ...
           xlist := rest xlist;
       >> else <<
           result := HearnFullGCD(result, first xlist);
           xlist := rest xlist;
       >>;
   >>;
   if not flag then
   ...
   while not(xlist = {}) do <<
       if numberp first xlist then <<
           ...
           >> else
               xlist := rest xlist;
       >> else <<
           if HearnFullGCD(myCont(poly, var), first xlist) = (first xlist) then <<
               poly := poly / (first xlist);
               xlist := rest xlist;
           >> else
               xlist := rest xlist;
